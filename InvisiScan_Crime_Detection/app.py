...
def analyze_video():
    ...
    location = ''.join([c if c.isalnum() or c in '-_' else '_' for c in location])
    save_dir = os.path.join('./uploaded_videos', location)
    os.makedirs(save_dir, exist_ok=True)
    video_path = os.path.join(save_dir, video.filename)
    video.save(video_path)
    # Example: Run your analysis logic here
    # For now, just return dummy activity and alert
    activity = 'Crime detection: No suspicious activity.'
    alert = False
    # TODO: Replace above with real analysis logic
    return jsonify({
        'message': f'Received video for analysis: {video_path}',
        'activity': activity,
        'alert': alert
    }), 200
